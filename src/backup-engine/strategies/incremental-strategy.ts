...
import { ChangeOperationType } from '../core/change-set.js';
import * as crypto from 'node:crypto';

export class IncrementalBackupStrategy implements BackupStrategy {
  public readonly strategyType = BackupJobType.INCREMENTAL;
  ...
  public async execute(params: {
    ...
    // 1. Localizar o último backup VÁLIDO da cadeia (Full ou Incremental anterior)
    const latestValidFull = await catalog.getLatestValidFullExecution(job.id);
    if (!latestValidFull) {
      throw new EngineError({
        code: 'CHAIN_BASE_FULL_MISSING',
        message: `Não foi possível executar backup INCREMENTAL: Nenhum backup FULL válido foi encontrado para o job "${job.name}".`,
    ...
    const latestValidInChain = await catalog.getLatestValidExecutionInChain(job.id, baseFullExecutionId);
    const parentExecution = latestValidInChain || latestValidFull;

    // Carregar estado efetivo consolidado até a execução pai
    const parentManifest = await catalog.consolidateEffectiveManifest(job.id, parentExecution.executionId);
    if (!parentManifest) {
      throw new EngineError({
        code: 'PARENT_MANIFEST_NOT_FOUND',
        message: `Manifesto da execução pai "${parentExecution.executionId}" não foi encontrado no catálogo.`,
        category: ErrorCategory.INTEGRITY,
        recoverable: false
      });
    }

    // Calcular sequência da cadeia
    const chainExecutions = await catalog.getChainExecutions(job.id, baseFullExecutionId);
    const sequence = chainExecutions.length; // 0=Full, 1=Inc1, 2=Inc2...
    const chainId = parentManifest.chainId || baseFullExecutionId;

    // 2. Fase de Varredura e Detecção de Alterações (ChangeDetector)
    progress.report({
    ...
    // Itens que requerem transferência (ADD, MODIFY, RENAME)
    ...
    // 4. Gravação e Validação do Manifest Incremental
    progress.report({
      executionId: execution.executionId,
      phase: 'FINALIZING',
      filesScanned: changeSet.totalScanned,
